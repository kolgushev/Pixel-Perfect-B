...
mat2x3 getLightColor(in vec3 lightAndAO, in vec3 normal, in vec3 normalViewspace, in vec3 sunPosition, in vec3 moonPosition, in int moonPhase, in int time, in float rain) {
    ...
    vec3 indirectLighting = max(vec3(0), ambientLight * (1 - clamp(lightmap.y * 1.5, 0, 1)) + torchLighting + ambientSkyLighting);
